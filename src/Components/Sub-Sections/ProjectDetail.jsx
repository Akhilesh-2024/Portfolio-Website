      ...
      <motion.div initial={{opacity:0, scale:0.5}} animate={{opacity:1, scale:1}} className="relative max-w-2xl border shadow-sm rounded-2xl bg-gradient-to-l from-midnight to-navy border-white/10">
        ...
        <div className="p-5">
          ...
          {subDescription.map((subDes,index) => (
            <p key={index} className="mb-3 font-normal text-neutral-500">{subDes}</p>
          ))}
          <div className="flex items-center justify-between mt-4">
            ...
            <a className="inline-flex items-center gap-1 font-medium hover-animation cursor-pointer" href={href}>View Project <img src="assets/arrow-up.svg" alt="Up Arrow" className="size-4" /></a>
          </div>
        </div>
      </motion.div>
    </div>
